    ...
    public ResponseEntity<FileUploadResponse> uploadFile(@RequestParam("file") MultipartFile file) {
        try {
            ...
            return ResponseEntity.ok(
                    new FileUploadResponse(200, "File uploaded successfully", fileName)
            );
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new FileUploadResponse(500, "Upload failed: " + e.getMessage(), null));
        }
    }
    ...
    public ResponseEntity<ByteArrayResource> downloadFile(@PathVariable String fileName) {
        try {
            ...
            return ResponseEntity.ok()
                    ...
                    .contentType(getMediaTypeForFileName(fileName)) // Détermine dynamiquement le type
    ...
    public ResponseEntity<FileUploadResponse> deleteFile(@PathVariable String fileName) {
        try {
            ...
            return new ResponseEntity<>(
                    new FileUploadResponse(200, "File deleted successfully", fileName),
                    HttpStatus.OK
            );
        } catch (Exception e) {
            return new ResponseEntity<>(
                    new FileUploadResponse(500, "Delete failed: " + e.getMessage(), fileName),
